...
`scripts/verify-release.ps1` performs mechanical release checks for version metadata, jar contents, local path leaks, resource JSON, player-facing product invariants, PNGs, asset contracts, language keys/placeholders, model texture references, optional `latest.log` server world-load evidence, and optional clean git working-tree evidence. It also verifies Forge, Minecraft, AE2, and GuideME dependency ranges in `mods.toml` against `gradle.properties`; confirms bundled README/CHANGELOG/LICENSE, language files, and Applied Packaging `assets/` and `data/` release resources match the current repository/generated sources; confirms local pattern compatibility items are not recipe/creative-tab outputs; confirms the package pattern terminal remains an AE2 `PartItem`; and supports `-RootPath` for temporary audit fixtures. It does not replace the Gradle, GameTest, manual client, or server smoke runs. Known external Yggdrasil public-key fetch failures are reported as warnings during log diagnostics; Applied Packaging, classloading, crash, missing texture, and other diagnostic keywords still fail the audit.

`scripts/run-release-checks.ps1` orchestrates the release check sequence: `build`, `runData`, `runGameTestServer`, optional `run-server-smoke.ps1`, the mechanical release audit, asset resource audit, documentation audit, optional tag-readiness audit, optional release manifest generation/audit, and optional release bundle generation/audit. Use `-ReleaseCandidate -RequireCleanGit` to run the full technical release-candidate gate. Use `-ReleaseCandidate -RequireCleanGit -RequireReadyForTag` only after the requirement/asset intake is frozen and all release changes are committed; this also requires `scripts/verify-release-readiness.ps1 -RequireReadyForTag` to pass. The readiness gate rejects pending, blocked, failed, unresolved-migration, missing-migration-target-path, traversal-migration-target-path, or misclassified requirement/asset migration-target intake rows and, once blockers are gone, requires explicit positive release signals in the docs: frozen scope, final dedicated-server world-load complete, release tag allowed, goal completable, and tag-readiness gate passed. Use `scripts/test-release-check-plan.ps1` after changing the release runner or release-candidate preset to self-test the plan order, every forbidden skip flag, and server world-load guardrails without running Minecraft. Use `scripts/test-release-readiness.ps1` after changing readiness rules to self-test ready, pending/blocked, failed-intake, unresolved-migration, missing-migration-target-path, traversal-migration-target-path, misclassified requirement/asset target, structural-failure, and missing-positive-signal fixtures without editing the real docs. Use `scripts/test-release-audit.ps1` after changing mechanical release audit rules to self-test valid release fixtures, missing required jar entries, stale bundled docs/language files, missing or stale bundled release resources, tampered `mods.toml` metadata, local path leaks, language placeholder mismatches, and product invariant regressions without running Minecraft. Use `scripts/verify-assets.ps1` after changing release PNG resources to verify required PNGs, known asset directories, RGBA PNG headers, visible non-placeholder pixel content, and expected dimensions. Use `scripts/test-assets-audit.ps1` after changing asset audit rules to self-test valid assets, bad dimensions, invalid PNG headers, fully transparent PNGs, single-color placeholder PNGs, and missing required PNGs. Use `scripts/test-release-manifest.ps1` after changing release manifest writer or audit rules to self-test valid manifests, tampered mod metadata, tampered jar hashes, and clean-git manifest checks. Use `scripts/test-release-bundle.ps1` after changing release bundle writer or bundle audit rules to self-test valid bundles, tampered manifest metadata, and tampered bundled docs. Use `scripts/test-docs-audit.ps1` after changing documentation audit rules to self-test valid docs fixtures, missing required paths, and broken local Markdown links. Use `scripts/test-release-self-tests.ps1` to run the docs-audit, asset-audit, release-audit, release-readiness, release-plan, manifest, and bundle self-tests together without starting Minecraft. `-ReleaseCandidate` rejects `-AuditOnly` and skip flags. When `-RunServerSmoke` is used, the server smoke runs after other Gradle runs, refreshes `run/logs/latest.log`, and the audit also requires dedicated server world-load evidence. Use `-RequireServerWorldLoad` only with `-AuditOnly` unless `-RunServerSmoke` is set. Use `-WriteReleaseManifest` to write `build/release/appliedpackaging-<version>-release-manifest.json` with jar size, SHA-256, version ranges, and git commit. Use `-RequireReleaseManifest` to verify that the manifest still matches the current jar, `gradle.properties`, and git HEAD. Use `-WriteReleaseBundle` and `-RequireReleaseBundle` to create and verify `build/release/appliedpackaging-<version>-release-bundle.zip` containing the jar, manifest, README, CHANGELOG, LICENSE, and SHA256SUMS; with `-RequireCleanGit`, the bundle audit also verifies that the bundled manifest git commit, branch, clean flag, and status lines match the current clean checkout.

Documentation audit self-tests also cover unresolved placeholder markers in formal design docs, so stale `TODO` / `FIXME` / `TBD` / `待定` / `等待 X` style text cannot quietly ship in the classified specs.

`scripts/verify-docs.ps1` checks that the required design, release, asset-brief, asset-contract, asset-report documents, and key release scripts exist, that `docs/design.md` and `docs/00-document-index.md` still cover the document set, that formal design docs contain no unresolved placeholder markers, and that local inline Markdown links resolve.
...
```text
compileJava:       passed
runGameTestServer: passed, 106 required GameTests
runData:           passed
build:             passed
...
release audit:     passed dependency metadata, source-synced release resources, asset contracts, and dedicated server world-load evidence
...
docs audit:        passed required document, placeholder, and local Markdown link checks
