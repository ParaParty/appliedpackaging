...
颜色模式是最左侧功能栏的第一枚按钮，只决定此后新增列的初始颜色，包括配方查看器填充新增的列；切换模式不会重新着色任何既有列。

- **默认**：新增列始终使用福鲁伊克斯色。
- **循环**：优先选择前面各列尚未使用的第一种包裹颜色；17 色全部出现后，从最后一列颜色的下一种继续循环并回绕。

转置会保留仍对应既有数据的列色，只有转置真正产生的新列使用当前颜色模式。每列自己的颜色按钮仍可用于明确修改。
...
配方查看器传输支持确定性的 JEI/EMI 配方，并通过同一颜色模式规则为新增列分配颜色。可选 Create 与 GTCEu 集成会保留已知物品/流体输入顺序；面对歧义或随机输出时会拒绝导入，避免静默生成错误计划。
