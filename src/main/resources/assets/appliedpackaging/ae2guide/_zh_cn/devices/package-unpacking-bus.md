...
包裹卸货总线从网络存储中接收包裹，并按编码顺序将内容物插入其所贴附的兼容存储目标。它支持与 AE2 样板供应器相同的可扩展外部存储目标，包括物品、流体，以及附属模组注册的其它 AE key 类型。

该总线也可作为有序供应器使用，类似于样板供应器将完整原料批次推送到相邻目标的方式。它会在推送前检查全部内容类型，并在交付时保留编码顺序。
...
*   可以启用阻挡模式，目标中只要已有任何内容就阻止拆包。目标必须完全清空，无关物品同样会阻挡。
