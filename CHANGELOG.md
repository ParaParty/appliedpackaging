...
- Mechanical release audit now checks bundled README, CHANGELOG, LICENSE, and language files against current repository sources.
...
- `scripts/test-release-audit.ps1` passed, confirming valid release audit fixtures pass and missing jar README, stale bundled docs/language files, tampered mod metadata, local path leaks, language placeholder mismatches, local pattern recipe outputs, creative-tab local patterns, or terminal `BlockItem` regressions fail.
