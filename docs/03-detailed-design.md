...
已编码 AE2 blank_pattern 通过客户端 tooltip hook 显示封装样板内容；普通 AE2 blank_pattern 不额外显示本 mod 文案。
AE2 encoded processing pattern 仍作为 colored_processing_pattern metadata 载体。
packaged_processing_pattern 当前仍使用本地物品；后续迁移到 AE2 原版 encoded pattern/Planner 语义前，必须保留已编码本地样板读取兼容或提供明确迁移路径。
```
...
已编码 AE2 blank_pattern 使用客户端 tooltip event 追加 package_pattern 内容；未编码 AE2 blank_pattern 保持 AE2 原版 tooltip。
...
输出保留输入样板物品类型；AE2 blank_pattern 会输出为带 package_pattern NBT 的 AE2 blank_pattern，AE2 processing pattern 会复制 1 个输出并写入 colored processing metadata。
...
默认初始选择为 Fluix。
当前已支持 AE2 原版 blank_pattern 作为 package_pattern 载体；仍不含 AE2 encoded pattern / Planner 语义集成、packaged_processing_pattern 的 AE2 原版载体迁移、流体/任意 AEKey 处理输出 ghost editor，也未实现真正 AE2 cable part 形态。
```
