...
`scripts/test-release-audit.ps1` 使用临时 release fixture 调用 `verify-release.ps1 -RootPath`，覆盖有效 release audit fixture、缺少 jar 必需 README 条目、`mods.toml` mod id 被篡改、jar 文本资源泄漏本机/reference 路径、语言占位符不一致、本地样板被 recipe 产出、创造栏暴露本地样板，以及包裹样板终端退回 BlockItem。该脚本不运行 Gradle、客户端或服务端，也不修改正式资源或发布产物。
