...
`scripts/verify-release-readiness.ps1` 检查 `docs/08-change-intake.md` 的新增项暂存表，以及本文件的目标完成/发布 tag 判定。默认模式用于预冻结审计，发现 blocker 时输出 WARN 但退出 0；使用 `-RequireReadyForTag` 时，任何待输入、待判定、阻塞或失败 intake、开放接收窗口、发布 tag 未完成或目标不能标记完成都会导致失败。当负面 blocker 全部清除后，脚本还要求 `docs/08-change-intake.md` 明确记录范围已冻结、最终服务端 world-load 已完成、发布 tag 可创建，并要求本文件明确记录可以标记完成、发布 tag 就绪门禁已通过；缺少这些正向信号也会阻止 tag。

`scripts/test-release-readiness.ps1` 使用临时 Markdown fixture 调用 `verify-release-readiness.ps1 -RequireReadyForTag`，覆盖 ready、pending/blocked、failed intake state、structural failure 和 missing positive signals 五种路径，确认 readiness 规则既能在范围冻结后放行，也能在待输入、阻塞/失败、结构缺失或只删除负面文字但缺少正向完成信号时失败。该脚本不修改正式设计文档。
