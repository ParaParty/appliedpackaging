...
当前最新执行：.\gradlew.bat runGameTestServer 成功，78 个必需 GameTest 全部通过。
```
...
Package Bus screen/menu 注册后，以及 Package Pattern Terminal 处理输出 ghost slots 布局调整后的客户端启动 smoke 通过。
