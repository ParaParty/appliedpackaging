...
world.menu / client.screen
  SequenceBufferMainMenu/Screen owns the endpoint 3x9 member window and row scrollbar
  SequenceBufferSideMenu/Screen owns the clicked member's single storage view
  both menus use the resolved endpoint as upgrade/filter authority and synchronize generic AEKey display stacks
  SequenceBufferBlock routes wrench interaction before menu opening and writes authority/viewed positions into the opening payload
...
-> output mode commits packages in strict queue order to the ME network, an adjacent item handler, or leaves them local; blocking mode requires the selected target to be empty before a drain starts, then the same server tick keeps draining the completed batch until the cache is empty or the target refuses another package
```
...
-> ordered multi-package plan using each column's color and marker; each package preserves row order and sparse row indexes in PackageLayout
```
...
-> when endpoint pattern mode is enabled, map layout positions to storage members (endpoint is not slot 1)
-> when pattern mode is disabled or layout is absent, use dense contents order without skipping positions
