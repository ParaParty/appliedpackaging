...
| --- | --- | --- | --- | --- | --- |
| IN-001 | 需求 | 包裹物品/实体同模型渲染、ME Packager 仅从固定底部与模型背面连接 AE2 MEStorage、空容量槽默认 1k 档 256/9 和完整打包流程 | 已迁移并通过当前验证 | docs/01-requirements.md; docs/02-system-architecture.md; docs/03-detailed-design.md; docs/05-implementation-plan.md; docs/06-verification-release.md | 2026-07-20 已运行 compileJava 与 runGameTestServer；当前验证覆盖底部/背面接网、扳手旋转、四朝向传送带点击和默认 1k 及容量元件档边界 |
...
| IN-008 | 需求/兼容 | 可选 JEI、EMI、Create、GTCEu/StarT Fork 与高级/包裹配方导入，包括序列组装和动力合成 | 已迁移并通过当前自动验证；Star Technology 重启后复验原生 EMI 与 JEMI 点击 | docs/01-requirements.md; docs/02-system-architecture.md; docs/03-detailed-design.md; docs/05-implementation-plan.md; docs/06-verification-release.md; docs/07-references.md | JEI/EMI 各自维护薄 handler/extractor，只共享 PatternTransferPlanFactory、领域数据、候选选择和 Create/GTCEu 语义编码；正式 viewer 路径直接调用公共编译 API，不反射发现模块、识别内部实现类或读取包装字段。原生 EMI 限定 FILL_BUTTON，公开 recipe recovery 依次使用 backing recipe、RecipeManager 和 GTCEu category registry。JEMI bridge 只按公开 recipe ID 的 `jei` namespace 让回 JEI transfer handler，两个查看器都不是发布硬依赖。通用、Create ProcessingRecipe 与 GTCEu item/fluid/tick input 默认每种材料一个包裹；Create 序列组装和动力合成保留专用分组，动力合成同时保留选中行/列的 sparse 前导与中间空位。自动验证结果见本轮开发日志；Star Technology 的两条实际按钮路径在替换 jar 并重启后完成。 |
