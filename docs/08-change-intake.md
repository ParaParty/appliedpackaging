...
| --- | --- | --- | --- | --- | --- |
| IN-001 | 需求 | 包裹物品/实体同模型渲染、ME Packager 仅从固定底部与模型背面连接 AE2 MEStorage、1k/16 基础容量和完整打包流程 | 已迁移并通过当前验证 | docs/01-requirements.md; docs/02-system-architecture.md; docs/03-detailed-design.md; docs/05-implementation-plan.md; docs/06-verification-release.md | 2026-07-15 已运行 compileJava 与 runGameTestServer；当前 106 个必需 GameTest 通过，覆盖底部/背面接网、扳手旋转与四朝向传送带点击 |
...
| IN-003 | 需求/材质 | 正式 UI/模型/动画调整 | 高级样板终端、原版样板终端包裹模式及全项目统一拾色弹窗已迁移并通过当前验证；其余范围待输入 | docs/01-requirements.md; docs/03-detailed-design.md; docs/04-asset-spec.md; docs/05-implementation-plan.md; docs/06-verification-release.md; docs/assets/reports/ui-and-icons.md; src/main/resources/assets/appliedpackaging/ | compileJava、历史 960x540 客户端截图、主界面/统一颜色弹窗/主产物 tooltip 截图人工与像素检查、日志关键字扫描、PNG 资源审计、资源/发布审计自测、文档审计、172 个必需 GameTest、build 和当时的发布审计均通过；原版包裹模式按新版贴图确认左侧滚动条、81 个 sparse 输入、3x3 可见窗口、marker 与输出布局；统一弹窗确认无标题、Fluix 左置、16 色右侧两行、底层 slot/item 保持渲染、无重复 tooltip/按钮消失/输入穿透；高级终端样板逻辑槽和编码按钮已对齐，历史客户端截图检查通过；两台机器与原版包裹模式空 marker 槽、升级区域和容量元件槽已统一到当前资源规格；等待用户视觉验收，其他 UI/模型/动画输入后分别按风险人工运行 runClient 复验 |
