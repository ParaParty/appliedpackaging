...
改发布包内容：运行 build，并执行 scripts/verify-release.ps1 审计 jar 条目、jar 内文档/语言源文件同步、文本资源路径、语言占位符和玩家入口产品不变量。
