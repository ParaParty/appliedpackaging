...
改客户端类或资源加载：运行 runClientSmoke，并检查 latest.log。
...
最终范围冻结后：执行 scripts/run-release-checks.ps1，必要时带 -RunClientSmoke；服务端 world-load 仍需手动运行 runServer 刷新 latest.log 后，再执行 scripts/run-release-checks.ps1 -AuditOnly -RequireServerWorldLoad 审计。
```
...
当前发布检查编排：scripts/run-release-checks.ps1 已加入；-RequireServerWorldLoad 已收敛为 -AuditOnly 专用模式；2026-07-04 执行 -RunClientSmoke 完整编排通过，执行 -AuditOnly -RequireAssetContracts -RequireServerWorldLoad 通过。
