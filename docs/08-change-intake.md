...
最终范围冻结后：执行 scripts/run-release-checks.ps1，必要时带 -RunClientSmoke；使用 -RunClientSmoke 时会自动审计 6 张 client smoke 截图。服务端 world-load 仍需手动运行 runServer 刷新 latest.log 后，再执行 scripts/run-release-checks.ps1 -AuditOnly -RequireServerWorldLoad 审计。全部变更提交后、发布 tag 前，执行 scripts/run-release-checks.ps1 -AuditOnly -RequireCleanGit。
```
