...
2026-07-11 当前非 UI 功能基线已收尾；正式 UI/模型/动画范围等待用户下一步描述，描述前不推测新资产用途。
...
当前验证基线：build、runData、runGameTestServer、runClientSmoke 均已有通过记录；2026-07-04 候选发布基线完成 dedicated server Done (2.471s) 和完整 ReleaseCandidate 门禁；当前非 UI 收尾基线已通过 170 个必需 GameTest、10 张必需 client smoke 截图、build、docs audit、release audit 与 clean-git audit。
...
| IN-003 | 需求/材质 | 正式 UI/模型/动画调整（范围与验收等待用户描述） | 待输入 | 待判定 | 待判定 |
