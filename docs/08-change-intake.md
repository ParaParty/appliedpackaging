...
改客户端类或资源加载：运行 runClientSmoke，并检查 latest.log 与 9 张必需 client smoke 截图。
...
当前验证基线：build、runData、runGameTestServer、runClientSmoke 均已有通过记录；2026-07-04 候选发布基线完成 dedicated server Done (2.471s) 和完整 ReleaseCandidate 门禁；2026-07-11 两种样板终端修正基线已通过 172 个必需 GameTest、11 张必需 client smoke 截图、build、docs audit、asset audit 与 release audit。
...
| IN-003 | 需求/材质 | 正式 UI/模型/动画调整 | 高级样板终端、原版样板终端包裹模式及全项目统一拾色弹窗已迁移并通过当前验证；其余范围待输入 | docs/01-requirements.md; docs/03-detailed-design.md; docs/04-asset-spec.md; docs/05-implementation-plan.md; docs/06-verification-release.md; docs/assets/reports/ui-and-icons.md; src/main/resources/assets/appliedpackaging/ | compileJava、960x540 client smoke、主界面/统一颜色弹窗/主产物 tooltip 截图人工与像素检查、六行网络库存中间重复段合成图检查、日志关键字扫描、149 个 PNG 资源审计、资源/发布审计自测、文档审计、172 个必需 GameTest、build 和含 11 张必需截图的发布审计均通过；原版包裹模式按新版贴图确认左侧滚动条、81 个 sparse 输入、3x3 可见窗口首槽 `(16,7)`、marker `(95,7)`、输出 `(98,31)`，tab 图标复用 AE2 完整 16px 单元和水平 tab 偏移，输入数量可见，空主产物 tooltip 与 AE2 一致；统一弹窗确认无标题、Fluix 左置、16 色右侧两行、底层 slot/item 保持渲染、弹窗 `z=400` managed batch 遮挡、无重复 tooltip/按钮消失/输入穿透；高级终端上下两个样板逻辑槽已各向右、向上修正 1px，编码按钮保持 `x=167` 并与槽内容/槽框同轴，新 client smoke 截图通过，用户 base PNG 字节未改；两台机器与原版包裹模式空 marker 槽已统一使用用户自绘 sprite、说明 tooltip 与 current-main hover，两台机器升级/工具箱区域和空升级槽已回移并通过隔离世界截图；两台机器容量元件空槽及装配室已编码样板空槽已改用 current-main `ae2-states.png` 切片；等待用户视觉验收，其他 UI/模型/动画输入后分别复验 |
...
| IN-005 | 需求/材质 | 删除 Package Export Bus/独立 Package Pattern Terminal；Storage/Unpacking Bus 七行过滤 GUI 与 AE2 part 迁移 | 行为与 GUI 底图已迁移并通过验证；正式 part 模型/新版卡图待后续输入 | docs/01-requirements.md; docs/02-system-architecture.md; docs/03-detailed-design.md; docs/04-asset-spec.md; docs/05-implementation-plan.md; docs/06-verification-release.md; src/main/resources/assets/appliedpackaging/ | compileJava、runData、174 个必需 GameTest、runClientSmoke、资源审计与发布审计自测通过；两个总线真实 AE2 part 截图已人工检查，过滤从底图第一行开始、三个按钮统一使用 2px 上边距、升级面板使用 current-main `top=0` 且仅显示 5 格；两个总线均允许该 5 格全部装入容量卡，第 6 张被拒绝，5 张容量卡按基础 2 行扩展到 7 行；Help 按钮和新版空升级槽占位已回移；用户更新的 Package Bus 底图已原字节接入，中心自定义区外与 current-main 可见像素差异为 0，源/运行时 SHA-256 均为 `506BE44EF826C14C1DBE37C076EDC7955C0DBFE35A7DB9B157EABA8E241787DE`；用户 sprite `(32,16,16,16)` 的自绘 marker 空槽图标、说明 tooltip 与窗口坐标 hover 修复已通过总线截图，sprite 字节未改；装入后的卡片物品贴图仍来自 AE2 15.4.10；卸货总线已接入真实 held 包裹、20 tick 进度、最终提交阻塞保留/重试、NBT 持久化与拆除返还；2026-07-15 最终以 105 个必需 GameTest 验证两个总线默认优先级均为 0、同值时卸货优先且忙碌后回落存储、玩家设置的更高数值仍优先、Formation Plane 式只写入接收、held 不可抽取、Pattern Provider 阻挡、颜色空模式和容器包裹分区；最终 server smoke 仍随发布候选门禁执行 |
| IN-006 | 材质/客户端 | ME Packager 正式空心框架、32x32 双周期传送带与四条帘子动画 | 已迁移并通过本轮验证 | docs/04-asset-spec.md; docs/assets/contracts/me_packager.yaml; docs/assets/reports/machines.md; src/main/resources/assets/appliedpackaging/ | asset contract、verify-assets、test-assets-audit、runData、100 个必需 GameTest、客户端 smoke、build 和 release asset-contract audit 均通过；朝向修正后的截图确认 `facing` 控制水平工作口、`network_side` 只控制接线，底盘不再因 up/down 接线翻转；15px 上表面+1px 正面连续 UV、四条帘子顶部转轴、包裹体积裁切和无 missing model 均通过，完整物品模型使用标准方块 display；三张用户 PNG 原字节 SHA-256 一致 |
