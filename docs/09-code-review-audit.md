...
- 删除把 AE2 `crafting_pattern` 伪装成包裹样板的旧载体、`colored_processing_pattern` / `packaged_processing_pattern` 扩展 NBT 解码，以及 AE2 blank pattern 兼容载体。正式自有载体只保留本 Mod `package_pattern` 与 `advanced_processing_pattern`；装配室另外直接接受 AE2 原生 crafting/processing/stonecutting/smithing 等可解码已编码样板。
