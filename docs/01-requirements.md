...
R7. ME 打包机必须支持一个可切换连接面的相邻 AE 网络识别，只通过该面连接 AE2 MEStorage；手动/红石触发打包、包裹拆包、源包裹展开后再封装、基础 1k 容量与 16 类型上限、颜色/marker 策略、打包内容过滤和拆包包裹过滤。其它面只暴露普通包裹输入/输出，不作为打包/拆包目标。
