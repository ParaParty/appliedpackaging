...
R6. ME 包裹装配室必须支持三种正式样板语义：包裹样板按样板定义生成一个包裹；普通 AE2 处理样板把样板输入生成一个默认 Fluix、空 marker 的包裹；高级处理样板按输入列颜色一次生成多个有序包裹，marker 固定取处理样板主产物。包裹不允许自定义名称。既有彩色处理样板与封装处理样板载体作为兼容路径保留；所有模式仍受合成进度、AE 能量、加速卡、严格有序单包输出和阻挡规则约束。
...
R8. 包裹样板终端必须支持编码包裹样板、给普通处理样板添加输入格颜色元数据、多个包裹样板合成封装处理样板、封装处理样板拆分回多个包裹样板；包裹样板还必须能在 AE2 原版 Pattern Encoding Terminal 中作为与 crafting / stonecutting / smithing 同级的包裹样板模式编码。原版终端的包裹模式使用高版本 AE2 124x66 模式面板，只显示清空、颜色按钮、3x3 可见输入窗口、左侧滚动条、marker 与自动输出；输入逻辑容量与 AE2 默认 processing pattern 相同，为 81 个 sparse 槽位，并支持数量显示、中键数量编辑、物品/流体容器和主产物 tooltip。高级样板终端复用 AE2 原版终端主体，只保留处理样板编码，显示 4 个可见输入列和 4 行输出，按列配置包裹颜色并由主产物提供 marker，编码独立的 `advanced_processing_pattern`。包裹不提供任何自定义名称入口或名称元数据。所有拾色入口复用同一个纯颜色高层弹窗，Fluix 位于最左，其余 16 色在右侧两行；弹窗不得重复 tooltip、隐藏底层物品或透传输入。
