...
已完成：
  ...
  scripts/verify-release-readiness.ps1 已作为 tag 就绪审计，-RequireReadyForTag 会阻止待输入、待判定、阻塞或失败 intake 状态下创建发布 tag，并在负面 blocker 清除后要求文档明确记录范围已冻结、最终服务端 world-load 已完成、发布 tag 可创建、目标可以标记完成和 tag 就绪门禁已通过
