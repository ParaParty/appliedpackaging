...
Both ScreenStyle files deliberately omit only the deferred 3x3 input-filter panel. The endpoint main screen alone uses the project's current-style AE2 vertical button bar for automatic output, blocking, anti-clog, synchronized output, pattern mode, and the `0/1/5/10/20/40/100 tick` input-delay presets. The member/standalone side screen has no adjustment controls; a formed member shows only the current-AE2 ENTER icon from `ae2-states.png [112,0]` to open the endpoint, while a standalone block shows no toolbar button. The one-slot redstone-card upgrade panel remains at `{right:2,top:0}`. The main scrollbar uses project-cached current-AE2 `ModernScrollbarStyles.BIG`: its 12x15 active or disabled handle starts at `(175,18)`, symmetrically covering the background track at `x=178..183`; a zero scroll range still renders the disabled handle. The asset audit fixes both PNG hashes/dimensions, the current big-scroller style, the absence of the rejected filter panel, and these main/side layout contracts.
