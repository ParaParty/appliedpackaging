...
### 2026-07-19 包裹样板网格尺寸与主网接线纠正

用户再次实机复核后确认，上一版固定为 4×4×4 的示例仍然错误。包裹样板网格现改为 2×2×2 棋盘核心，包含 4 个样板供应器和 4 个包裹装配室；结构边界额外保留一格智能线缆，明确作为接入主网络的连接端。对应双语装配室页和基础装配线页统一改为 4 个频道，不再要求致密线缆。文档结构门禁同步锁定 2×2×2 核心、4+4 方块、单根主网智能线缆及其位于核心外的连接位置，并把 4×4×4 改为失败夹具。

### 2026-07-19 序列缓存器双输入与打包机网络示例纠正

序列缓存器结构移除能源元件，并按两种输入来源分别提供场景：包裹卸货总线接入端点；扳手定向的样板供应器把选定输出面朝向端点。两张结构都保留输入设备接回其 ME 网络的智能线缆，但序列缓存器本身不使用 AE 能源。机器页与有序机器输入示例页同步展示两张结构，没有改写其余行为说明。

ME 打包机新增“箱子、普通 AE2 存储总线、智能线缆、ME 打包机、能源元件”结构，打包机通过底面接入线缆。结构审计明确禁止用包裹存储总线替代普通存储总线；序列缓存器审计则明确禁止两种结构出现能源元件，并分别检查样板供应器与包裹卸货总线不会混入另一张场景。`scripts/verify-docs.ps1`、完整 `scripts/test-docs-audit.ps1`、`git diff --check` 与 `.\gradlew.bat build --no-configuration-cache` 均通过。GuideME 开发源映射分别以 `zh_cn`、`en_us` 加载 34 个资源页面中的对应语言 17 页并逐页编译，新结构与页面没有 GuideME、structure 或 SNBT 解析错误；`run/options.txt` 已恢复为 `zh_cn`。本轮只修改 GuideME 文档、SNBT 场景和文档审计，不涉及服务端行为，因此没有重跑 GameTest。

### 2026-07-19 序列缓存器示例 1：四路并行熔炉组

按用户提供的实机布局，在“有序机器输入”页新增并行熔炉组，而不是替换前面的两种通用输入结构。样板供应器内置一个真实 AE2 加工样板：输入 1 为煤炭 ×1，输入 2 为粗铁 ×8，输出为铁锭 ×8；选定输出面朝向包裹装配室。装配室把整批原料封成一个包裹并写入白色处理子网，四个包裹卸货总线分别连接四套序列缓存器支路。每套支路的成员 1 向下交给侧面燃料漏斗，成员 2 从顶面送入粗铁 ×8，熔炉下方的输入总线再收回铁锭。

网络按实际频道拆成三组：粉色主网只连接样板供应器；白色处理子网包含包裹装配室与四个包裹卸货总线，共 5 个频道；橙色回收子网包含四个输入总线和一个对着样板供应器的普通存储总线，同样为 5 个频道。两个子网都只通过石英纤维从主网取电，不使用能源元件，也不直接与主网合并频道。样板供应器的定向输出面不建立网格连接，因此装配室所在子网不会并回主网；橙色存储总线通过供应器的返回库存提交加工结果。

`scripts/verify-docs.ps1` 新增该结构的固定语义门禁，逐项检查样板 NBT、四套卸货总线/端点/两成员/漏斗/熔炉/输入总线、普通存储总线、两处石英纤维、三色智能线缆和无能源元件；`scripts/test-docs-audit.ps1` 新增把粗铁 ×8 改成 ×1 的失败夹具。`scripts/verify-docs.ps1`、完整 `scripts/test-docs-audit.ps1`、`git diff --check` 与 `.\gradlew.bat build --stacktrace --no-configuration-cache` 通过。GuideME 20.1.7 使用隔离的 `run-gtceu-fork`、开发源映射和 `zh_cn` 启动，加载 34 个双语资源中的中文 17 页并逐页编译，目标页及 `sequence_furnace_array.snbt` 没有页面、结构或 SNBT 编译错误；验证后只结束隔离客户端，用户 IntelliJ 客户端 PID 22796 未被停止。本轮仅修改 GuideME 文档、场景和文档审计，已检查现有 GameTest source set，但没有服务端行为变化，因此不重跑 GameTest。

### 2026-07-19 序列缓存器示例 2：9×9 动力合成

“有序机器输入”页继续保留原有两种通用输入结构和示例 1，另行新增 9×9 动力合成场景。样板供应器内置一个带空位的 9×9 高级样板，九列分别编码为九种颜色；选定输出面朝向包裹装配室，装配室开启阻挡模式并输出到处理子网。九个按颜色过滤的包裹卸货总线分别接入九条序列缓存器，每条由一个端点和九个定向成员组成，端点同时开启自动输出、样板模式和同步输出。场景用交替的普通箱子与陷阱箱逐格代替 81 个动力合成器，避免相邻箱子合并，也不把 Create 方块写成指南结构的硬依赖。

处理子网实际包含包裹装配室与九个包裹卸货总线，共占 10 个频道，因此结构使用致密智能线缆作为主干，再从每层接出普通智能线缆安装总线；总线没有与致密线缆共用 CableBus 方块。粉色主网只连接样板供应器，处理子网只通过石英纤维取电；序列缓存器不包含能源元件。页面正文只补充该场景的配置、空位保持和频道说明，没有改写前面的专门优化文案。

`scripts/verify-docs.ps1` 对 9×9 高级样板元数据、九色列、空位、阻挡模式、九个拆包支路、9 个端点、81 个成员、81 个箱子、10 频道致密主干、普通线缆支路、石英纤维和无能源元件逐项固定；`scripts/test-docs-audit.ps1` 新增关闭装配室阻挡模式的失败夹具。隔离客户端完成模组初始化、资源重载、配方加载并正常退出；本轮早先已用 GuideME 实际解析定位并修正过结构括号和“总线装在致密线缆 CableBus”两类错误，最终拓扑不再出现对应结构 NBT 警告，但最后一次启动没有留下逐页编译日志，因此不记作完整页面验收。最终门禁以 `scripts/verify-docs.ps1`、完整 `scripts/test-docs-audit.ps1`、`.\gradlew.bat build --stacktrace --no-configuration-cache` 和 `git diff --check` 为准。本轮仅修改 GuideME Markdown、SNBT 与文档审计；已检查现有 GameTest source set，没有服务端行为变化，因此不新增或重跑 GameTest。

### 2026-07-19 示例 3：ME 打包机颜色路由

“有序机器输入”页在示例 2 后新增独立路由场景，没有改动前面的通用结构、熔炉组或 9×9 动力合成。橙色路由网络连接四个包裹存储总线，分别启用红、蓝、绿、黄单色过滤；每个总线贴在一台 ME 打包机上，打包机的颜色选择与对应总线严格一致。四台打包机另一侧各保留一段粉色智能线缆，表示四个互不相连的目标网络；橙色网络顶部另留一个接口连接包裹来源。

包裹进入橙色网络后，只会尝试颜色匹配的包裹存储总线，并由该总线送入对应打包机；打包机再将内容拆入自己的粉色目标网络。页面明确说明总线和打包机必须同色，并提醒默认 Fluix 在打包机上代表不限制包裹颜色，不适合需要严格分流的支路。场景没有能源元件，电源和目标库存由结构边界外的真实网络提供。

`scripts/verify-docs.ps1` 将“有序机器输入”的场景数量固定为 5，并逐坐标检查四个总线颜色序号与四台打包机的 `selected_color`、橙色路由网络、粉色目标接口、包裹来源接口和无能源元件。`scripts/test-docs-audit.ps1` 新增把黄色打包机改为黑色的失败夹具，确认总线与打包机不一致时审计必定失败。新结构已用 Minecraft 1.20.1 `TagParser` 实际解析，得到 3 个尺寸值和 33 个结构条目；`scripts/verify-docs.ps1`、完整 `scripts/test-docs-audit.ps1`、`.\gradlew.bat build --stacktrace --no-configuration-cache` 与 `git diff --check` 全部通过。本轮仍只修改 GuideME Markdown、SNBT 与文档审计；已检查现有 GameTest source set，没有服务端行为变化，因此不新增或重跑 GameTest。

### 2026-07-19 示例 4：高级样板多步骤合成

“有序机器输入”页在示例 3 后继续新增独立的多步骤合成场景，没有改动前四张通用或专用结构。样板供应器内置一个四列高级样板，四列分别设置为红、蓝、绿、黄；选定输出面朝向包裹装配室。装配室按列生成四个完整包裹并输出到白色处理子网，四个包裹存储总线使用与样板列一致的颜色过滤，将包裹分别写入四个步骤输入箱。场景中的材料和箱子只作为分组及步骤入口占位，不把包裹存储总线误写成直接拆出散装内容。

白色子网包含包裹装配室与四个包裹存储总线，共占 5 个频道，并只通过石英纤维从粉色主网取电。页面同时区分了列顺序和加工依赖：列顺序只决定装配室生成包裹的顺序，颜色决定路由目的地；真正的步骤依赖仍需由上一步产物返回 ME 网络后触发下一份加工样板，不能依赖包裹输出顺序代替。

`scripts/verify-docs.ps1` 将目标页的场景数量提高为 6，并固定四列高级样板 v2 元数据、四个颜色及其总线坐标、一个包裹装配室、四个步骤输入箱、白色 5 频道子网、粉色主网、石英纤维和无能源元件。`scripts/test-docs-audit.ps1` 新增把黄色总线过滤改为其他颜色的失败夹具。新结构已用 Minecraft 1.20.1 `TagParser` 实际解析，得到 3 个尺寸值和 16 个结构条目；`scripts/verify-docs.ps1`、完整 `scripts/test-docs-audit.ps1` 与 `.\gradlew.bat build --stacktrace --no-configuration-cache` 通过。本轮只修改 GuideME Markdown、SNBT 与文档审计；已检查现有 GameTest source set，没有服务端行为变化，因此不新增或重跑 GameTest。

### 2026-07-19 有序机器输入示例顺序与熔炉优先级补充

按用户指定成对交换示例 1/2 与 3/4，双语页面当前顺序统一为：示例 1“9×9 动力合成”、示例 2“并行熔炉组”、示例 3“多步骤合成”、示例 4“路由”。四段场景、标注和原有说明按完整章节移动，没有把不同示例的结构或配置交叉改写。

动力合成示例补充水平摆放路径：先从 JEI 把配方填入高级样板编码终端，再使用“转置配方”交换包裹列与材料行，使包裹列对应水平缓存行；产物不随转置改变。并行熔炉结构的四个包裹卸货总线从左到右设置为优先级 4、3、2、1，网络先尝试能够完整接收包裹的最高优先级支路，忙碌时依次回退到后续支路。

`scripts/verify-docs.ps1` 新增双语示例顺序、JEI 转置说明和四个总线坐标/优先级门禁；`scripts/test-docs-audit.ps1` 新增把末路优先级 1 改回 0 的失败夹具。修改后的熔炉结构已由 Minecraft 1.20.1 `TagParser` 解析，得到 3 个尺寸值和 41 个结构条目；`scripts/verify-docs.ps1`、完整 `scripts/test-docs-audit.ps1` 与 `.\gradlew.bat build --stacktrace --no-configuration-cache` 通过。现有 GameTest source set 已覆盖包裹总线高优先级目的地优先于低优先级目的地的行为；本轮没有修改 Java 或服务端逻辑，因此不新增或重跑 GameTest。

### 2026-07-19 动力合成示例缩减为 5×5

按实机展示需求，将原 9×9 动力合成场景整体缩减为 5×5。高级样板现包含五个带空位的有序列，分别使用 Fluix、白、橙、品红和淡蓝五种颜色；处理子网对应使用五个包裹卸货总线、五个序列缓存器端点和 25 个定向成员。包裹装配室继续开启阻挡模式，端点继续开启自动输出、样板模式和同步输出，JEI 填充后使用高级样板编码终端“转置配方”的水平摆放说明保持不变。

25 个替代动力合成器的箱子全部放在序列缓存器成员的正面，并按普通箱子与陷阱箱交替摆放以避免相邻箱子合并；结构不再把箱子放在成员背面或侧面。处理子网现只有包裹装配室与五个卸货总线，共占 6 个频道，因此移除原 10 频道场景的致密智能线缆，统一使用普通白色智能线缆。粉色主网、石英纤维供电边界和序列缓存器无能源元件的约束保持不变。

`scripts/verify-docs.ps1` 同步固定 5×5 高级样板、五色过滤、5 个端点、25 个成员、25 个正面箱子、6 频道普通智能线缆和无致密线缆；`scripts/test-docs-audit.ps1` 新增把首个箱子移到成员背后的失败夹具。新结构已由 Minecraft 1.20.1 `TagParser` 解析，得到 3 个尺寸值和 71 个结构条目；`scripts/verify-docs.ps1` 与完整 `scripts/test-docs-audit.ps1` 通过。现有 GameTest source set 与运行任务已检查；本轮仍只修改 GuideME Markdown、SNBT 和文档审计，没有 Java 或服务端行为变化，因此不新增或重跑 GameTest。

### 2026-07-19 有序机器输入示例拆分为独立页面

“有序机器输入”双语页面不再同时承载四套大型示例。父页只保留包裹卸货总线输入、样板供应器输入、通用配置、工作原理和“一次一个”规则，并通过 `SubPages` 列出四个子页。5×5 动力合成、并行熔炉组、多步骤合成和颜色路由分别迁移到独立的中英文页面，继续按示例 1～4 和导航位置 10、20、30、40 排列；原有场景、标注及经过优化的正文只做完整移动，没有重新改写表达。
